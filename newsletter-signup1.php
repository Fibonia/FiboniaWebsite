...
db.collection("users").where("email", "==", "'.$id.'")
  .get()
  .then(function(querySnapshot) {
      querySnapshot.forEach(function(doc) {
          db.collection("users").doc(doc.id).update({newsletter: true}).then(function(tst){window.location.replace("https://www.fibonia.com/newsletternew.php");})
      });
 })
</script>';
?>