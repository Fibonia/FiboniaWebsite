...
db.collection("users").where("email", "==", "'.$_POST["emailentered"].'")
  .get()
  .then(function(querySnapshot) {
      querySnapshot.forEach(function(doc) {
          db.collection("users").doc(doc.id).update({newsletter: false}).then(function(tst){window.location.replace("https://www.fibonia.com/fib.php");})
      });
 })
</script>';
?>
